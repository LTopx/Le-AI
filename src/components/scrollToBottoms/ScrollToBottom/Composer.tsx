...
export interface ComposerProps extends React.PropsWithChildren {
...
const emotionPool: any = {};

function setImmediateInterval(fn: any, ms: number) {
  fn();
...
function isEnd(animateTo: any, mode: "top" | "bottom") {
...
const Composer = ({
  ...
  const initialScrollBehaviorRef = useRef<any>(initialScrollBehavior);
  const [animateTo, setAnimateTo, animateToRef] = useStateRef(
    mode === MODE_TOP ? 0 : "100%"
  );
  ...
  const scrollPositionObserversRef = useRef<any[]>([]);
  ...
  const handleSpineToEnd = useCallback(() => {
    ...
    setAnimateTo(null);
  }, [animateToRef, ignoreScrollEventBeforeRef, mode, setAnimateTo, setSticky]);

  // Function context
  const scrollTo = useCallback(
    (nextAnimateTo: any, { behavior } = { behavior: "smooth" }) => {
      const { current: target } = targetRef;
    ...
    [handleSpineToEnd, mode, setAnimateTo, setSticky, targetRef]
  );
  ...
  const scrollToSticky = useCallback(() => {
    ...
    if (target) {
      if (initialScrollBehaviorRef.current === "auto") {
        target.scrollTop =
          mode === MODE_TOP ? 0 : target.scrollHeight - target.offsetHeight;
      ...
      const maxValue =
        mode === MODE_TOP
          ? 0
          : Math.max(0, scrollHeight - offsetHeight - scrollTop);
      const minValue = Math.max(0, animateFrom - scrollTop);

      const rawNextValue = scroller({
        maxValue,
        minValue,
        offsetHeight,
        scrollHeight,
  ...
  }, [animateFromRef, mode, scroller, scrollTo, targetRef]);
  ...
  const handleScroll = useCallback(
    ({ timeStampLow }: any) => {
      ...
      const { atBottom, atEnd, atStart, atTop } = computeViewState({
        mode,
        target,
      });
  ...
  useEffect(() => {
    if (target) {
      let stickyButNotAtEndSince: any = false;
  ...
  const styleToClassName = useMemo(() => {
    const emotion =
      emotionPool[nonce] ||
      (emotionPool[nonce] = createEmotion({
        key: "react-scroll-to-bottom--css-" + createCSSKey(),
        nonce,
      }));

    return (style: any) => emotion.css(style) + "";
  }, [nonce]);

  const internalContext: any = useMemo(
    () => ({
  ...
  const functionContext: any = useMemo(
    () => ({
      scrollTo,
      scrollToBottom,
      scrollToEnd,
