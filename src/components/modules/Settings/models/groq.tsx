...
export function Groq() {
  ...
  const [groq, setGroq] = useModelsStore((state) => [
    state.groq,
    state.updateGroq,
  ])

  const onRemove = () => {
    setGroq({ key: '', endpoint: '', error: true })
  ...
  const validate = () => {
    ...
    const requestInfo = getRequestInfo('groq', key, endpoint)
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (res.code || res.error) {
          ...
          setGroq({ key, endpoint, error: false })
        }
      })
      .catch(() => {
        toast.error(
    ...
    <Card>
      <CardHeader className="p-[18px]">
        <h4 className="flex scroll-m-20 items-center gap-2 text-lg font-semibold tracking-tight">
          <Image src="/models/groq.svg" alt="groq" width={75} height={20} />
          {groq.error ? (
      ...
      <CardContent className="px-[18px] pb-[18px]">
        <div className="grid w-full items-center gap-4">
          ...
          <div className="flex flex-col space-y-1.5">
            <Label htmlFor="groq-endpoint">API Endpoint</Label>
            <Input
              id="groq-endpoint"
              ref={endpointRef}
              placeholder="https://api.groq.com/openai"
              defaultValue={groq.endpoint}
